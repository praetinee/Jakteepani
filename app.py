...
from data_special_rules import special_rules # นำเข้าข้อมูลเกณฑ์พิเศษ
...
with col_right:
    ...
    # 1. เช็คเกณฑ์พิเศษ (Special Rules) ก่อนเป็นอันดับแรก
    if selections_names:
        matched_rules = []
        for rule in special_rules:
            is_match = True
            for req_house, req_planets in rule["conditions"].items():
                # ดึงรายการดาวที่มีอยู่ในภพนั้น (ถ้าไม่มีให้เป็นลิสต์ว่าง)
                user_planets_in_house = selections_names.get(req_house, [])
                # เช็คว่าดาวที่ต้องการ (req_planets) มีครบอยู่ในภพนั้นหรือไม่
                for p in req_planets:
                    if p not in user_planets_in_house:
                        is_match = False
                        break # ออกจากลูปย่อยทันทีถ้าขาดไป 1 ดวง
                
                if not is_match:
                    break # ข้ามไปตรวจกฎข้อถัดไป
            
            if is_match:
                matched_rules.append(rule)
                
        # แสดงผลกฎเกณฑ์พิเศษที่ตรวจพบ
        if matched_rules:
            for match in matched_rules:
                special_html = (
                    f'<div style="font-family: \'Sarabun\', sans-serif; background-color: #fef2f2; border: 2px solid #f87171; border-left: 8px solid #dc2626; padding: 20px; border-radius: 10px; margin-bottom: 24px; box-shadow: 0 4px 10px rgba(220, 38, 38, 0.15);">'
                    f'<h4 style="color: #b91c1c; margin-top: 0; margin-bottom: 12px; font-size: 18px;">🚨 คำทำนายเกณฑ์พิเศษ (พินทุบาทว์ / ดาวผสม)</h4>'
                    f'<span style="background-color: #fee2e2; color: #991b1b; padding: 4px 10px; border-radius: 6px; font-weight: bold; font-size: 14px; margin-bottom: 10px; display: inline-block;">{match["name"]}</span>'
                    f'<p style="font-size: 18px; color: #7f1d1d; margin: 0; line-height: 1.6; margin-top: 8px;"><strong>คำทำนาย:</strong> {match["text"]}</p>'
                    f'</div>'
                )
                st.markdown(special_html, unsafe_allow_html=True)
    
    # 2. คำนวณหาว่าแต่ละภพตรงกับราศีอะไร
    house_zodiac_map = {}
    ...
    # 3. แสดงผลคำทำนายรายภพ และ ราศี (แบบปกติ)
    if selections_names:
