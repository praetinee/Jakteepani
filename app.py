...
# โครงสร้างฐานข้อมูล (แปลงสีเป็นรหัสสี Hex สำหรับใช้งานใน Streamlit)
astrology_database = {
...
# สร้างช่องเลือก (Selectbox) สำหรับแต่ละภพ
for position, planets_data in astrology_database.items():
    options = ["-- ไม่ระบุดาว (ว่าง) --"] + list(planets_data.keys())
    choice = st.selectbox(f"**{position}**", options, key=position)
    
    if choice != "-- ไม่ระบุดาว (ว่าง) --":
        selections[position] = choice
...
if selections:
    ...
    for pos, planet in selections.items():
        result_data = astrology_database[pos][planet]
        
        # ใช้ HTML และ CSS ตกแต่งกล่องข้อความให้สวยงามคล้ายโค้ด React เดิม
        html_card = f"""
        <div style="
